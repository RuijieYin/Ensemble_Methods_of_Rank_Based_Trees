...
#'
#' @examples
#' rules_table <- extract.rules(data = train_data,n.trees = 10)
...
select.rules <- function(validation.data = validation_data, rule.table = rules_table$Rules) {
  # in validation data: rows are samples and columns are vars
  colnames(validation.data)[dim(validation.data)[2]] = "subtype"
